...
class OverviewViewModel : ViewModel() {
    ...
    //Add update filter function for processing filter on all, but and rent properties on Mars
    fun updateFilter(filter: MarsApiFilter) {
        getMarsRealEstateProperties(filter)
    }

    //add a displayPropertyDetails() method that sets _navigateToSelectedProperty to the selected Mars property.
    fun displayPropertyDetails(marsProperty: MarsProperty) {
        _navigateToSelectedProperty.value = marsProperty
    }
    //Add a displayPropertyDetailsComplete() method that nulls the value of _navigateToSelectedProperty
